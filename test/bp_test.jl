...
using BasePlateFEA
using LinearAlgebra, SparseArrays, Statistics, GLMakie
...
A_cs, yCG, zCG, Iyr, Iy, Izr, Iz = cs_data(h, b, c, tb, th, tc)
...
# BOLT HOLES
# ─────────────────────────────────────────────────────────────────────────────
HOLE_table = Vector{Union{Nothing, HoleData}}(nothing, nplate)

x1 = L - Da; y1 = W/2 - Sa/2
x2 = L - Da; y2 = W/2 + Sa/2
sec = 4*3  # polygon sides (multiple of 4)
dalp = 2*pi / sec
alp0 = 0.0
...
push!(CONDSUP_table, Any["surf", 1, 0.0, 0.0, L, W, 0.0, 0.0, 100000.0, 0.0, 0.0, 0.0, "g", -1.0])

# ─────────────────────────────────────────────────────────────────────────────
# ANALYSIS
# ─────────────────────────────────────────────────────────────────────────────
nodes, el_nodes, el_props, nodenr, pl_el_edges, pl_edge_nodes =
    main_modelgen(PLATE_table, LOAD_table, SUPPORT_table, CONDSUP_table, HOLE_table,
...
for iincr in 1:nincr
    global d_ini, Ke, lv
    d_stat, Ke, lv = FEanal_incr(iincr, dincr, d_ini, Ke, lv,
    ...
    if stressout > 0.01 && iincr == nincr
        ...
        out_res = main_stressout(d_out, PLATE_table, CONDSUP_table, MAT_table,
        ...
        fig1 = plot_res_tri(el_nodes, el_props, nodes, FE_dat, out_res, scale_vis, layer, 3, true;
...
ax4  = Axis(fig4[1, 1];
            xlabel = "Baseplate max vertical displacement (in)",
            ylabel = "Moment (pound-in)",
            title  = "Force-Displacement")
lines!(ax4, dmami[:, 2], M2 * 1000 .* lf[1:end]; color=:blue, linewidth=2)
save(joinpath(@__DIR__, "..", "force_disp.png"), fig4)
